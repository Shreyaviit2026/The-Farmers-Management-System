...
def edit(rid):
    if request.method == "POST":
        ...
        try:
            # Validate the inputs
            validate_user_details(phonenumber, adharnumber, age)
...
def validate_user_details(mobile_no, aadhaar_no, age):
...
def register():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            query = Register(farmername=farmername, adharnumber=adharnumber, age=int(age),
                             gender=gender, phonenumber=phonenumber, address=address, farming=farmingtype)
