...
('arkpro', 'arkpro@gmail.com', 3, 'silk', 'silk is best business developed from coocon for sarees preparation and so on', 582);
...
-- Example of a cursor for processing rows
DELIMITER //

CREATE PROCEDURE ProcessFarmingData()
BEGIN
  DECLARE done INT DEFAULT 0;
  DECLARE fid INT;
  DECLARE farmingtype VARCHAR(255);
  DECLARE cur CURSOR FOR SELECT `fid`, `farmingtype` FROM `farming`;
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = 1;

  OPEN cur;

  read_loop: LOOP
    FETCH cur INTO fid, farmingtype;
    IF done THEN
      LEAVE read_loop;
    END IF;
    -- Process each row here
    -- Example: INSERT INTO another_table (fid, farmingtype) VALUES (fid, farmingtype);
  END LOOP;
...
-- Products Table (Primary key is `pid`)
...
-- User Table (References `pid` from Products table)
CREATE TABLE `user` (
...
INSERT INTO `products` (`pid`, `productname`, `productdesc`, `price`) VALUES
(1, 'GIRIJA CAULIFLOWER', 'Tips for Growing Cauliflower. Well-drained medium loam and/or sandy loam soils are suitable.', 520),
(2, 'COTTON', 'Cotton is a soft, fluffy staple fiber that grows in a boll, around the seeds of the cotton.', 563),
(3, 'SILK', 'Silk is a business developed from cocoons for saree preparation and so on.', 582);

-- Insert Data into User Table
INSERT INTO `user` (`id`, `username`, `email`, `password`, `pid`) VALUES
(5, 'arkpro', 'arkpro@gmail.com', 'pbkdf2:sha256:150000$TfhDWqOr$d4cf40cc6cbfccbdcd1410f9e155ef2aa660620b0439a60c4d74085dbf007a4a', 3),
(6, 'test', 'test@gmail.com', 'someencryptedpassword', 1);


INSERT INTO user (username, email, password, pid) 
VALUES ('sid123', 'sid@gmail.com', '12345', 1); -- Assume 1 exists in `products`.


-- SELECT * FROM products;?

ALTER TABLE user MODIFY pid INT DEFAULT NULL;

-- INSERT INTO user (username, email, password) 
-- VALUES ('sid123', 'sid@gmail.com', '12345');



ALTER TABLE register
ADD CONSTRAINT chk_mobile_no CHECK (LENGTH(phonenumber) = 10);

ALTER TABLE register
ADD CONSTRAINT chk_aadhaar_no CHECK (LENGTH(adharnumber) = 12);
